 ...
 *
 * Market-wide sentiment only: VIX + MOVE + Fear & Greed.
 * BTC-specific sentiment (derivatives, on-chain, ETF flow) is handled in btc-signal.ts.
 *
...
export function analyzeSentimentSignal(db: Db, date: string): void {
	...
	// Weighted composite (VIX 35% + MOVE 25% + Fear&Greed 40%)
	const compositeScore =
		vixScore * SENTIMENT_WEIGHTS.vix + moveScore * SENTIMENT_WEIGHTS.move + fgScore * SENTIMENT_WEIGHTS.fearGreed;
