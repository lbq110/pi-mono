...
| 模块 | 阶段 | 说明 |
|------|------|------|
| **日报增强 §8-§10** | Phase 3F ✅ | 持仓回顾、相关性与轮动、交易信号详解 |
| **L2-L4 风控** | Phase 3E+ ✅ | 回撤分级 5%/10%/15%，信用否决，BTC -5% 联动 |
| **仓位管理** | Phase 3 ✅ | ATR 仓位、移动止损、相关性惩罚、1/4 Kelly |
...
**L2-L4（✅ 已实现）：**
- L2：回撤分级 5%/10%/15% → 风控乘数 ×0.5/×0.25/×0（含阶梯恢复 + 双倍退防）
- L3：信用利差否决 → `credit_risk = risk_off_confirmed` 时权益仓位归零
- L4：BTC -5% 24h 急跌 → 权益仓位减仓 20%，12h 冷却
...
### Phase 3 已完成（续）
- 子任务 E（L2-L4）：回撤分级 + 阶梯恢复 + 双倍退防 + 信用否决 + BTC 急跌联动
- 子任务 F：日报增强 §8 持仓回顾、§9 相关性与轮动、§10 交易信号详解（含 ATR、Kelly、信号分解）
